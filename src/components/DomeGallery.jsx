...
function buildItems(pool, seg, verticalSpan) {
  const xCols = Array.from({ length: seg }, (_, i) => -37 + i * 2);

  const makeYs = (span, odd) => {
    const start = odd ? -span + 1 : -span;
    const end = odd ? span + 1 : span;
    const ys = [];
    for (let y = start; y <= end; y += 2) ys.push(y);
    return ys;
  };

  const evenYs = makeYs(verticalSpan, false);
  const oddYs = makeYs(verticalSpan, true);
...
export default function DomeGallery({
  ...
  const items = useMemo(() => buildItems(images, segments, verticalSpan), [images, segments, verticalSpan]);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      // Enhanced height guard to prevent dome from being too small
      const heightGuard = h * 0.9; // Increased from 1.35 to allow larger domes
